...
from moneycontrol import moneycontrol_api as mc # <-- NEW: Moneycontrol Import
...
# --- NEW: Moneycontrol News Function ---
@st.cache_data(ttl="1h")
def fetch_moneycontrol_news(symbol):
    """Fetches general business news from Moneycontrol."""
    try:
        # We ignore the specific ticker search since that is often blocked. 
        # We fetch general business news instead, which is usually allowed.
        mc_client = mc.API() 
        news_data = mc_client.get_business_news() 
        
        # Structure the data (moneycontrol-api returns a list of dictionaries)
        structured_news = []
        for item in news_data[:5]: # Only take the top 5 general news items
             # Note: moneycontrol-api returns "Title" and "Link" (Source 1.1)
        ...
        # Fails silently and logs error to console
        print(f"Silent Error fetching Moneycontrol news: {e}") 
        return []
...
def create_model(df):
    ...
    y_train, y_test = y.iloc[:split_index], y.iloc[split_index:] # Corrected line
...
if st.button("Analyze & Predict"):
    with st.spinner(f"Analyzing {stock_symbol} (using 1 year of data)..."):
        try:
            ...
            # --- NEW: Call Moneycontrol for news ---
            news = fetch_moneycontrol_news(stock_symbol) 
            ...
            if hist_df is None:
                ...
                with st.container(border=True):
                    st.subheader("Long-Term AI Recommendation (12+ Months)") 
                ...
                with st.container(border=True):
                    ...
                    with col2_news:
                        st.subheader("Recent News (Moneycontrol)") # Updated title
                        if news:
                            for item in news[:5]: # Show top 5 items
                            ...
                            st.write("No recent news found (Moneycontrol may be blocking access).") # Updated fail message
